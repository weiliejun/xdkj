layui.use(['form', 'upload', 'layer', 'laytpl', 'jquery'], function () {
    ...
    $(function () {
        $.ajax({
            url: PageContext.getUrl("/deviceInfo/detail/" + $("#ryId").val()),
            type: 'post',
            async: false,
    ...
    var active = {
        selectSpr: function () {
            Common.openFrame("/deviceInfo/selectList?cxmk=selectSpr&ryid=" + $("#sprId").val(), "选择审批人", '1000px', '600px');
        },
        selectZhr: function () {
            Common.openFrame("/deviceInfo/selectList?cxmk=selectZhr&ryid=" + $("#zhrId").val(), "选择知会人", '1000px', '600px');
        },
        add: function () {
