...
jQuery.validator.addMethod("rightUserName", function(value, element) {
	...
	return this.optional(element) || (length == 11 && mobile.test(value)) || /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/.test(value);
}, "请输入正确的邮箱/手机号");

// 昵称验证
jQuery.validator.addMethod("nickName", function(value, element) {
	value = $.trim(value);
	return this.optional(element) || (/^(?!^\d+$)[a-zA-Z0-9_]{6,25}$/.test(value));
}, " 用户名由英文字母、数字、下划线组成,长度请保持6-25位");
...
jQuery.validator.addMethod("pwCheck", function(value, element) {
    return this.optional(element) || /^(?![0-9]+$)(?![a-zA-Z]+$)(?![_()\@\!\#\$\%\^\&\*\.\~]+$)[0-9A-Za-z_()\@\!\#\$\%\^\&\*\.\~]{6,16}$/.test(value);
}, "请输入6~16位字母、数字和符号组合！");
...
jQuery.validator.addMethod("checkCharLength", function(value, element) {
	return this.optional(element) || (getLength($.trim(value)) <= 32 && getLength($.trim(value)) >= 3);
}, "长度请保持4-32个字符以内");
// 身份证验证
...
/** 自定义表单校验 ,有一个校验错误就停止校验**/
function myFormValidate(validator){
	var flag = false;
	validator.elements().each(function(index,element){
		flag =  validator.element(element);
        if(!flag) return flag;
	});
	return flag;
}
